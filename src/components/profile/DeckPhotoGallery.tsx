...
const DeckPhotoGallery: React.FC<DeckPhotoGalleryProps> = ({
  deckId,
  initialPhotos = [],
  onSave
}) => {
  ...
  return <div className="space-y-6 mx-[41px] my-[33px]">
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="caption" placeholder="Añade una descripción para esta foto..." value={caption} onChange={e => setCaption(e.target.value)} />
            </div>
            
            <div className="space-y-2">
              <Label htmlFor="photo">Seleccionar foto</Label>
              <Input ref={fileInputRef} id="photo" type="file" accept="image/*" onChange={handleFileUpload} />
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div>
        ...
        {photos.length === 0 ? <div className="text-center py-8 border rounded-md">
            ...
            {photos.map(photo => <Dialog key={photo.id}>
                <DialogTrigger asChild>
                  <div className="relative aspect-square rounded-md overflow-hidden cursor-pointer border hover:shadow-md transition-shadow" onClick={() => setSelectedPhoto(photo)}>
                    <img src={photo.url} alt={photo.caption || 'Deck photo'} className="w-full h-full object-cover" />
                    {photo.caption && <div className="absolute bottom-0 left-0 right-0 bg-background/80 backdrop-blur-sm px-2 py-1 text-xs truncate">
                        {photo.caption}
                      </div>}
                  </div>
                </DialogTrigger>
