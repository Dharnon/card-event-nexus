...
const SideboardGuideComponent: React.FC<SideboardGuideProps> = ({ deckId, initialGuide, onSave }) => {
  ...
  const [activeMatchup, setActiveMatchup] = useState<string | null>(
    guide.matchups.length > 0 ? guide.matchups[0].id : null
  );
      ...
      {isEditing ? (
        <div className="space-y-6">
          <Card>
            ...
            <CardContent>
              <Textarea 
                value={guide.mainNotes} 
                onChange={(e) => setGuide(prev => ({ ...prev, mainNotes: e.target.value }))}
                placeholder="Escribe notas generales sobre tu mazo y su estrategia..."
                rows={5}
              />
            </CardContent>
          </Card>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <div className="flex space-x-2">
                <Input 
                  placeholder="Nombre del matchup" 
            ...
            {guide.matchups.length > 0 ? (
              <Tabs value={activeMatchup || ''} onValueChange={setActiveMatchup} className="w-full">
                ...
                {guide.matchups.map((matchup) => (
                  <TabsContent key={matchup.id} value={matchup.id} className="mt-6">
                    <Card>
                      ...
                      <CardContent className="space-y-4">
                        <div className="space-y-2">
                          ...
                          <Textarea 
                            value={matchup.strategy} 
                            onChange={(e) => updateMatchup(matchup.id, { strategy: e.target.value })}
                            placeholder="Describe tu estrategia para este matchup..."
                            rows={3}
                          />
                        </div>
                        ...
                        <div className="grid gap-6 md:grid-cols-2">
                          <div className="space-y-2">
                            ...
                            <div className="space-y-4">
                              <CardSearchInput 
                                onCardSelect={(card) => addCardToMatchup(matchup.id, card, true)}
                                placeholder="Buscar carta para meter..."
                              />
                              
                              <div className="border rounded-md p-3 space-y-2 min-h-[100px]">
                                {matchup.cardsToSideIn.length === 0 ? (
                                  <p className="text-sm text-muted-foreground">No hay cartas seleccionadas</p>
                                ) : (
                                  matchup.cardsToSideIn.map((card) => (
                                    <div key={card.id} className="flex justify-between items-center">
                                      <span>{card.name}</span>
                                      <Button 
                                        variant="ghost" 
                                        size="sm" 
                                        onClick={() => removeCardFromMatchup(matchup.id, card.id, true)}
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="space-y-4">
                              <CardSearchInput 
                                onCardSelect={(card) => addCardToMatchup(matchup.id, card, false)}
                                placeholder="Buscar carta para sacar..."
                              />
                              
                              <div className="border rounded-md p-3 space-y-2 min-h-[100px]">
                                {matchup.cardsToSideOut.length === 0 ? (
                                  <p className="text-sm text-muted-foreground">No hay cartas seleccionadas</p>
                                ) : (
                                  matchup.cardsToSideOut.map((card) => (
                                    <div key={card.id} className="flex justify-between items-center">
                                      <span>{card.name}</span>
                                      <Button 
                                        variant="ghost" 
                                        size="sm" 
                                        onClick={() => removeCardFromMatchup(matchup.id, card.id, false)}
                                        className="h-6 w-6 p-0"
                                      >
                                        <Trash className="h-4 w-4" />
                                      </Button>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent>
              {guide.mainNotes ? (
                <div className="whitespace-pre-line">{guide.mainNotes}</div>
              ) : (
                <p className="text-muted-foreground">No hay notas generales</p>
              )}
            </CardContent>
          </Card>
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-6">
                        <div>
                          ...
                          {matchup.strategy ? (
                            <div className="whitespace-pre-line">{matchup.strategy}</div>
                          ) : (
                            <p className="text-muted-foreground">No hay estrategia definida</p>
                          )}
                        </div>
                        ...
                        <div className="grid gap-6 md:grid-cols-2">
                          <div>
                            ...
                            {matchup.cardsToSideIn.length === 0 ? (
                              <p className="text-muted-foreground">No hay cartas seleccionadas</p>
                            ) : (
